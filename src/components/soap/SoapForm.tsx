        ...
        <div className="w-full max-w-5xl mx-auto p-4 md:p-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8 border-b border-slate-200 dark:border-white/5 pb-8">
                ...
                <div className="flex gap-3 w-full md:w-auto">
                    ...
                    <button onClick={onCancel} className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-red-500/10 hover:bg-red-500/20 text-red-500 px-3 py-2 sm:px-6 sm:py-3 rounded-2xl text-[9px] sm:text-[10px] font-black uppercase tracking-[0.2em] transition-all border border-red-500/10">
                        <span className="material-symbols-outlined text-base sm:text-lg">close</span> {isSimulation ? 'Salir' : 'Descartar'}
                    </button>
                </div>
            </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="bg-neutral-900 border border-white/5 rounded-[24px] p-5 flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <input
                                        value={report.referencia_viaje}
                                        onChange={(e) => setReport({ ...report, referencia_viaje: e.target.value })}
                                        className="bg-transparent text-white font-black text-sm uppercase tracking-tight outline-none border-b border-transparent focus:border-primary/30 w-full"
                        ...
                        <div className="bg-neutral-900 border border-white/5 rounded-[32px] p-8 relative overflow-hidden group">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Paciente (Nombre)</label>
                                    <input readOnly={!isSimulation} value={patientName} className={`w-full bg-white/5 border border-white/5 rounded-2xl p-4 text-sm text-white outline-none focus:ring-1 focus:ring-primary/50 transition-all ${!isSimulation ? 'opacity-60' : ''}`} placeholder="Nombre completo" />
                                </div>
                                <div className="space-y-2">
